...
public final class PackUiScrollList<T> {
    ...
    public final PackUiScrollViewport viewport;
    public final Function<T, String> labelExtractor;
    public final BiConsumer<T, Boolean> selectionHandler;
    public final List<T> items = new ArrayList<>();
    public int selectedIndex = -1;
    public boolean enabled = true;

    public int borderColor = PackUtilColors.secondary();
    public int backgroundColor = PackUtilColors.listBg();
    public int selectedTextColor = PackUtilColors.rowSelectedText();
    public int normalTextColor = 0xFFFFFFFF;
    public int disabledTextColor = PackUtilColors.textDim();

    public Function<T, String> badgeExtractor;
    public Function<T, Integer> badgeColorExtractor;

    public Function<T, String> secondaryTextExtractor;
    public int secondaryTextColor = PackUtilColors.textDim();
    ...
    public void renderContent(GuiGraphicsExtractor ctx, Font textRenderer, double mouseX, double mouseY) {
        viewport.renderSimple(ctx, items.size(), (index, bounds) -> {
            T item = items.get(index);
            boolean isSelected = index == selectedIndex;
            boolean isHovered = enabled && isHovered(mouseX, mouseY, bounds.x, bounds.y, bounds.width, bounds.height);
